...
class create_user:
    ...
    def update_client(cpf):
        ...
        values = {"address": address, "account": {"type_account": type_account}}

        try:
            filter_cpf = {"cpf": cpf}

            result = db_pymongo.collection.update_one(
                filter_cpf, {"$set": {"address": address, "account.type": type_account}}
            )
